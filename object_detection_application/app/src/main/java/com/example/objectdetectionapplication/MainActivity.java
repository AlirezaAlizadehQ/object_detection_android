package com.example.objectdetectionapplication;
...
public class MainActivity extends AppCompatActivity implements CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    boolean start_detect = false;
    boolean first_time_detect = false;
    Net detect_model;

    public void DETECT(View Button){
        // when the DETECT button is clicked
        if (!start_detect){
            start_detect = true;
            if (!first_time_detect){
                first_time_detect = true;
                // access the external storage for the model
                ...
                // read the model from the internal files
                detect_model = Dnn.readNetFromDarknet(tinyYoloCfg, tinyYoloWeights);
            }
        }
        else{
            start_detect = false;
    ...
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        ...
        if (start_detect) {
            ...
            detect_model.setInput(imageBlob);
            java.util.List<Mat> result = new java.util.ArrayList<Mat>(2);
            List<String> outBlobNames = new java.util.ArrayList<>();
            outBlobNames.add(0, "yolo_16");
            outBlobNames.add(1, "yolo_23");
            detect_model.forward(result,outBlobNames);
            ...
            if (ArrayLength>=1) {
                ...
                for (int idx : ind) {
                    ...
                    List<String> cocoNames = Arrays.asList("a person", "a bicycle", "a motorbike",
                            "an airplane", "a bus", "a train", "a truck", "a boat", "a traffic light",
                            "a fire hydrant", "a stop sign", "a parking meter", "a car", "a bench",
                            "a bird", "a cat", "a dog", "a horse", "a sheep", "a cow", "an elephant",
                            "a bear", "a zebra", "a giraffe", "a backpack", "an umbrella", "a handbag",
                            "a tie", "a suitcase", "a frisbee", "skis", "a snowboard", "a sports ball",
                    ...
                    Imgproc.putText(frame, cocoNames.get(idGuy) + " " + intConf + "%", box.tl(),
                            Core.FONT_HERSHEY_SIMPLEX, 2, new Scalar(255, 255, 0), 2);
    ...
    protected void onResume() {
        ...
        if (!OpenCVLoader.initDebug()){
            Toast.makeText(getApplicationContext(),"OOpsss, problem occured!", Toast.LENGTH_SHORT).show();
        }
        else
        {
